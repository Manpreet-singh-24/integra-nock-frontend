...
export function* createOrganisation(action) {
    ...
    try {
        yield put({type: LOADER_OPEN});
        // yield put({type: types.DATA_LOADED_STATUS});
        const result = yield call(post,`chaincode/list`, action.payload.data);
...
export default function* organisationSaga() {
    yield all([ listingReq(), signOrganisationRequest(), checkUpdateRequest(), installChainCodeRequest() ]);
}
